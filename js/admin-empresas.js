...
async function clearAllInDB() {
  ...
  // ====== Descarga respaldo Excel por año y periodo ======
  const btnDescargar = document.getElementById("btnDescargarRespaldo");
  const inputAnio    = document.getElementById("respaldoAnio");
  const selPeriodo   = document.getElementById("respaldoPeriodo");
  const estadoResp   = document.getElementById("estado-respaldo");

  function setEstadoRespaldo(msg, color) {
    if (!estadoResp) return;
    if (!msg) { estadoResp.style.display = "none"; estadoResp.textContent = ""; return; }
    estadoResp.style.display = "block";
    estadoResp.style.backgroundColor = color === "ok"
      ? "rgba(21,128,61,0.25)"
      : color === "error"
        ? "rgba(185,28,28,0.25)"
        : "rgba(255,255,255,0.1)";
    estadoResp.style.color = "#fff";
    estadoResp.textContent = msg;
  }

  async function descargarRespaldoExcel() {
    // Validar año
    const anio = (inputAnio?.value || "").trim();
    if (!anio || isNaN(Number(anio)) || Number(anio) < 2020 || Number(anio) > 2100) {
      setEstadoRespaldo("⚠ Ingresa un año válido (2020-2100).", "error");
      inputAnio?.focus();
      return;
    }
    ...
    if (!periodo) {
      setEstadoRespaldo("⚠ Selecciona un periodo.", "error");
      selPeriodo?.focus();
      return;
    }

    setEstadoRespaldo("Consultando datos en Supabase...", "info");

    // Traer todos los registros (sin modificar nada)
    let { data, error } = await supa.from(TABLE).select("*").limit(10000);
    if (error) {
      console.error(error);
      setEstadoRespaldo("Error al consultar datos: " + (error.message || "desconocido"), "error");
      return;
    }

    const rows = data || [];
    if (rows.length === 0) {
      setEstadoRespaldo("No hay registros en la base de datos.", "error");
      return;
    }

    setEstadoRespaldo(`Generando Excel con ${rows.length} registros...`, "info");

    // Construir filas — teléfonos como texto (prefijo ' para evitar que Excel los trunque)
    const encabezados = [
      "NOMBRE",
      "DIRECCION",
      "TELEFONO",
    ...
    rows.forEach(r => {
      ...
      const telefono = get(["telefono", "TELEFONO"]);

      aoa.push([
        get(["nombre",                    "NOMBRE"]),
        get(["direccion",                  "DIRECCION"]),
        telefono,                          // se guardará como texto vía cell z
        get(["nombre_carta",              "NOMBRE_CARTA"]),
        get(["puesto_carta",              "PUESTO_CARTA"]),
        get(["correo_cont",               "CORREO_CONT"]),
        get(["programa_educativo_solicitado", "PROGRAMA_EDUCATIVO_SOLICITADO", "programa", "PROGRAMA"]),
        get(["giro_de_la_empresa",        "GIRO_DE_LA_EMPRESA", "giro", "GIRO"]),
      ]);
    });
    ...
    // Forzar columna TELEFONO (col C = índice 2) como texto
    const range = XLSX.utils.decode_range(ws["!ref"]);
    for (let rowIdx = 1; rowIdx <= range.e.r; rowIdx++) {
      const cellAddr = XLSX.utils.encode_cell({ r: rowIdx, c: 2 });
      if (ws[cellAddr]) {
        ws[cellAddr].t = "s"; // string
        ws[cellAddr].z = "@"; // formato texto
      }
    }
    ...
    // Nombre del archivo: Empresas_{AÑO}_{PERIODO}_{FECHA}.xlsx
    const hoy = new Date();
    const fechaStr = `${hoy.getFullYear()}-${String(hoy.getMonth() + 1).padStart(2, "0")}-${String(hoy.getDate()).padStart(2, "0")}`;
    const nombreArchivo = `Empresas_${anio}_${periodo}_${fechaStr}.xlsx`;

    XLSX.writeFile(wb, nombreArchivo);

    setEstadoRespaldo(`✅ Descargado: ${nombreArchivo} (${rows.length} registros)`, "ok");
  }
  ...
  document.addEventListener("DOMContentLoaded", async () => {
    ...
    if (btnDescargar) btnDescargar.addEventListener("click", descargarRespaldoExcel);
